...
- `DINGTALK_WEBHOOK_2`：第二个钉钉群机器人的 Webhook。
- `DINGTALK_SECRET_2`：第二个钉钉机器人的加签密钥。
...
3. 在 GitHub 仓库添加第一组 `DINGTALK_WEBHOOK`、`DINGTALK_SECRET`；第二组使用 `DINGTALK_WEBHOOK_2`、`DINGTALK_SECRET_2`。
...
大正抓取每小时读取公开报名列表并复核详情页报名按钮，同时检查官方新闻公告中的报名开放、截止和补录/名额消息。会员招募、教练培训、等级考试和明确的海外站不会作为国内赛事写入赛历。
